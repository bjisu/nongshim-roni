...
2. **파워** — 초록→노랑→빨강 게이지가 왕복. 탭하는 순간의 파워로 슈팅
   - 너무 약하면 골대까지 못 가고, 너무 강하면 골대 위로 넘어감 (적정 구간: 게이지 40~85%)
3. **판정** — 방향이 골대 안쪽 범위 + 파워가 적정 구간이면 골인(+100). 골키퍼 로니는 연출 전용(골이면 반대로, 빗나가면 공 쪽으로 다이빙)
4. **제한시간** — 슈팅 1회당 10초. 초과하면 기회 1 차감 (3초 이하부터 빨간 경고)
5. **결과** — 3회 슈팅 후 총점(최대 300)·골 수·로니 리액션 표시
...
| `KEEPER_ZONE_SPLIT` | 골키퍼 다이빙 방향 구간 경계 (좌/중/우 3등분) |
